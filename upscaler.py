...
def upscale_image_bytes(img_bytes: bytes, orig_format: str, scale: float, method: str, jpeg_quality: int, keep_format: bool) -> Optional[bytes]:
    methods = []
...
def process_docx_replace(input_docx: str, scale: float = 2.0, method: str = "pillow", jpeg_quality: int = 95, keep_format: bool = True, output_docx: Optional[str] = None) -> str:
    _check_imports()
...
def cli():
    parser = argparse.ArgumentParser(description="Upscale and REPLACE embedded images inside a .docx using python-docx.")
    ...
    parser.add_argument("--method", choices=["pillow", "realesrgan", "ncnn"], default="pillow", help="Upscaling backend")
